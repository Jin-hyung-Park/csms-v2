...
**배경:** 신규 근로자 가입 시 수습기간이 수동으로 설정되어야 해서 누락 위험이 있었음. 법정 수습 기간(3개월)을 가입 즉시 자동 적용하도록 개선.

**수정 내용:**

| 파일 | 변경 |
|------|------|
| `server/src/routes/auth.route.js` | 근로자(`role !== 'owner'`) 가입 시 `hiredAt`(입사일)을 가입 당일, `probationEndDate`(수습 종료일)를 가입일 + 90일로 자동 설정. 점주 가입은 해당 없음. |

**동작:**
- 기존 가입 직원: 영향 없음
- 신규 가입 직원: 가입 즉시 `hiredAt`, `probationEndDate` 자동 저장
- 점주가 직원 상세 페이지에서 수습 종료일 수동 수정 가능 (기존 기능)

---

### 2026-07-13 — 점주 급여 상세 화면 시급 적용 내역 표시

**배경:** 점주가 급여 산정 후 상세 화면에서 각 직원에게 시급이 어떻게 적용되었는지 확인할 수 없었음. 특히 수습기간 전환이 있는 직원의 경우 최저시급/계약시급 혼용 여부를 파악하기 어려운 문제.

**수정 내용:**

| 파일 | 변경 |
|------|------|
| `server/src/routes/monthlySalary.route.js` | `GET /:userId/:year/:month` 응답에 `probationEndDate`, `minWage` 추가. 각 일별 근무(`dailySchedules`)에 `wageApplied`(적용 시급), `isProbation`(수습 여부) 필드 추가. |
| `client/src/pages/owner/SalaryDetail.jsx` | 상단에 "시급 적용 정보" 섹션 추가(계약 시급·수습 종료일 표시). 주차별로 수습/계약 적용 일수 배지 및 기본급 셀에 `시급 × 시간` 계산식 표시. 일별 근무 목록에 적용 시급 및 수습 여부 표시. |

**표시 위치:**
1. **시급 적용 정보 섹션** (상단) — 계약 시급 + 수습 종료일 / 수습기간 없음 여부
2. **주차별 배지** — 수습 N일(최저시급) / 계약 M일(계약시급) 구분
3. **기본급 계산식** — `12,000원 × 10h` 또는 수습 혼용 시 `10,320원×5h + 12,000원×5h`
4. **일별 근무** — 각 근무일 옆 적용 시급 및 수습 여부 표시

---

### 2026-07-06 (2) — 복지포인트 월 귀속 기준 변경 (월요일 → 일요일)

**배경:** 월 경계 주차(예: 6/29~7/5)에서 복지포인트가 월요일 기준으로 전월(6월)에 귀속되어, 7월 급여 미리보기에 해당 주의 복지포인트가 ₩0으로 표시되는 문제. 일요일 기준으로 변경하여 마지막 주차는 복지포인트 미표기, 다음 달 첫 주차에 이전 월 근무 포함한 전체 주 시간으로 산정하도록 수정.

**산정 규칙 변경:**
- 기존: 해당 주의 **월요일**이 속한 월에 복지포인트 귀속
- 변경: 해당 주의 **일요일**이 속한 월에 복지포인트 귀속 (주휴수당 귀속 기준과 동일)
- 예시: 6/29(월)~7/5(일) 주차 → 일요일 7/5가 7월 → **7월 급여**에 6/29, 6/30 포함 전체 주 근무시간으로 복지포인트 산정

**수정 내용:**

| 파일 | 변경 |
|------|------|
| `server/src/routes/owner.route.js` | 복지포인트 산정 조건 `startsInPrevMonth → endsInNextMonth` 변경 |
| `server/src/routes/monthlySalary.route.js` | 동일 조건 변경 및 주석 업데이트 |
...
| 파일 | 변경 |
|------|------|
| `client/src/pages/owner/Salary.jsx` | 기간 컬럼 날짜 포맷을 `YYYY-MM-DD` → `MM/DD`로 변경 (`toMD` 헬퍼 추가). 주차별 근무시간 셀에 해당 주 최종 시급 표시 추가. 수습기간(`probationEndDate`) 기준으로 주 마지막 날이 수습 중이면 최저시급(10,320원), 이후면 계약 시급을 표시 (`getWeekWage` 헬퍼). 서버 계산 로직(`workDate < probationEnd`)과 동일 기준 적용. |
