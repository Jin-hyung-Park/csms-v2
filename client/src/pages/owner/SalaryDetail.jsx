...
export default function OwnerSalaryDetailPage() {
  ...
  // 근로자 확인 완료 취소 mutation (수정 사항 반영 시 사용)
  const unconfirmEmployeeMutation = useMutation({
    mutationFn: async (salaryId) => {
      const { data } = await apiClient.put(`/monthly-salary/${salaryId}/unconfirm-employee`);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['monthly-salary-detail']);
      queryClient.invalidateQueries(['monthly-salaries']);
      alert('근로자 확인 완료가 취소되었습니다. 수정 후 근로자에게 다시 확인을 요청해 주세요.');
    },
    onError: (error) => {
      const message = error.response?.data?.message || '근로자 확인 취소 중 오류가 발생했습니다.';
      alert(message);
    },
  });
  ...
  const handleUnconfirmEmployee = () => {
    if (data.status === 'confirmed') return;
    if (window.confirm('근로자 확인 완료를 취소하시겠습니까? 수정 후 근로자가 다시 급여를 확인해야 합니다.')) {
      unconfirmEmployeeMutation.mutate(data._id);
    }
  };
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {data.employeeConfirmed && !isConfirmed && (
              <>
                ...
                <button
                  type="button"
                  onClick={handleUnconfirmEmployee}
                  disabled={unconfirmEmployeeMutation.isPending}
                  className="touch-target rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition active:bg-slate-50 hover:bg-slate-50 disabled:opacity-50"
                >
                  {unconfirmEmployeeMutation.isPending ? '처리 중...' : '확인 완료 취소'}
                </button>
              </>
            )}
            {isAdjusted && (
