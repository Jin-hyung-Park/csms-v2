...
| **근로자1** | `employee1@test.com` | `password123` | 판교역점 소속 (승인됨) |
| **근로자2** | `employee2@test.com` | `password123` | 수지구청점 소속 (승인됨) |
| **승인 대기** | `pending@test.com` | `password123` | 가입 요청 상태 (점주 승인 필요) |
...
근로자 회원가입 시 위 매장코드를 입력하면 해당 매장으로 가입 요청됩니다.
...
#### 1.3 직원 관리 및 가입 승인 테스트

1. 하단 네비게이션에서 "직원" 메뉴 클릭
2. 확인 사항:
   - 직원 목록 표시 (승인됨/승인 대기 배지)
   - 점포별 필터, **승인 상태 필터** (전체 / 승인 대기 / 승인됨)
   - 각 직원의 통계 (근무시간, 승인 대기/승인 건수)
3. **가입 요청 승인** (승인 대기 직원):
   - 필터에서 "승인 대기" 선택
   - 승인 대기 중인 직원 카드에서 "상세보기" 클릭
   - 시급·세금 유형 확인/수정 후 **가입 승인** 버튼 클릭
   - 승인 후 해당 직원은 "승인됨" 상태로 변경
4. **기존 직원 정보 수정**:
   - "상세보기" 클릭 후 **시급·세금 유형** 수정 → "저장" 버튼
   - **근로 요일·근로 시간**: 요일별 체크박스(근무일 여부), 시작 시간/종료 시간 입력 후 "저장" 버튼
...
2. 점포 등록:
   - "+ 점포 등록" 버튼 클릭
   - 점포명, 주소 입력 (필수)
   - **매장코드 (5자리, 선택)**: 근로자 회원가입 시 사용할 코드 (예: AB123). 미입력 시 해당 점포로 매장코드 가입 불가.
   - 전화번호, 사업자번호, 설명 입력 (선택)
   - "등록하기" 버튼 클릭
...
2. 다음 입력:
   - 이름, 이메일, 비밀번호, 비밀번호 확인
   - **역할**: 근로자
   - **매장코드 (5자리)**: `PG001` 또는 `SJ002` (시드 데이터 기준)
3. "검증" 버튼으로 매장코드 확인 (매장명 표시되면 유효)
4. "회원가입" 클릭
5. 안내: "가입 요청이 완료되었습니다. 점주 승인 후 서비스를 이용할 수 있습니다."
6. 자동 로그인 후 직원 대시보드로 이동 → **"가입 승인 대기 중입니다"** 배너 표시

#### 3.2 점주: 가입 요청 승인

1. 점주(`owner@test.com`)로 로그인
2. **직원** 메뉴 → 필터 **"승인 대기"** 선택
3. 방금 가입한 직원 카드에서 **상세보기** 클릭
4. 시급·세금 유형 확인/수정 후 **가입 승인** 버튼 클릭
5. 승인 후 해당 직원은 "승인됨" 상태로, 정상적으로 서비스 이용 가능
