...
describe('인증 API 통합 테스트', () => {
  let testUser;
  let testToken;
  let testStoreCode = 'TX999'; // 근로자 회원가입 테스트용 매장코드

  beforeAll(async () => {
    await connectDB();
    await User.deleteMany({ email: /@test\.com$/ });
    await Store.deleteMany({ storeCode: testStoreCode });
    const owner = await User.create({
      name: '테스트 점주',
      email: 'owner-auth-test@test.com',
  ...
  describe('POST /api/auth/register', () => {
    it('회원가입을 할 수 있어야 함', async () => {
      const payload = {
        ...
        phone: '010-1234-5678',
        role: 'employee',
        storeCode: testStoreCode, // 근로자는 5자리 매장코드 필수
      };
