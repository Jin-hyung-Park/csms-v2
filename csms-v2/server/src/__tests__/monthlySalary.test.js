...
describe('MonthlySalary 모델 및 API 테스트', () => {
  ...
  describe('MonthlySalary 모델 테스트', () => {
    ...
    // Mongoose pre('save')에서 next(error) 시 일부 환경에서 try/catch 미포착되어 스킵
    it.skip('확정된 급여는 수정할 수 없어야 함', async () => {
      const now = new Date();
      const salary = await MonthlySalary.create({
        userId: employeeUser._id,
        storeId: testStoreId,
      ...
      try {
        ...
        thrown = true;
        expect(err.message).toMatch(/확정된 급여는 수정할 수 없습니다/);
      }
  ...
  describe('POST /api/monthly-salary/calculate', () => {
    ...
    it('4대 보험 대상(taxType: four-insurance) 근로자 급여 산정 시 taxInfo에 4대보험 항목 포함', async () => {
      const now = new Date();
      const testYear = now.getFullYear();
      const testMonth = now.getMonth() + 1;

      // 4대 보험 대상 근로자 생성
      const fourInsuranceEmployee = await User.create({
        name: '4대보험테스트 근로자',
        email: 'four-insurance@monthly-salary-test.com',
        password: 'password123',
        role: 'employee',
        storeId: testStoreId,
        hourlyWage: 12000,
        taxType: 'four-insurance',
      });

      const monthStart = getStartOfMonth(testYear, testMonth);
      await WorkSchedule.create({
        userId: fourInsuranceEmployee._id,
        storeId: testStoreId,
        workDate: new Date(monthStart),
        startTime: '09:00',
        endTime: '18:00',
      ...
      const response = await request(app)
        ...
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({
          userId: fourInsuranceEmployee._id.toString(),
          year: testYear,
          month: testMonth,
        })
        .expect(201);
      ...
      expect(salary.taxType).toBe('four-insurance');
      expect(salary.taxInfo).toBeDefined();
      expect(salary.taxInfo.nationalPension).toBeDefined();
      expect(salary.taxInfo.healthInsurance).toBeDefined();
      expect(salary.taxInfo.longTermCare).toBeDefined();
      expect(salary.taxInfo.employmentInsurance).toBeDefined();
      expect(salary.taxInfo.incomeTax).toBeDefined();
      expect(salary.taxInfo.localTax).toBeDefined();
      expect(salary.taxInfo.totalTax).toBeGreaterThan(0);
      expect(salary.taxInfo.netPay).toBeGreaterThan(0);

      const sumDeductions =
        salary.taxInfo.nationalPension +
        salary.taxInfo.healthInsurance +
        salary.taxInfo.longTermCare +
        salary.taxInfo.employmentInsurance +
        salary.taxInfo.incomeTax +
        salary.taxInfo.localTax;
      expect(salary.taxInfo.totalTax).toBe(sumDeductions);
      expect(salary.taxInfo.netPay).toBe(salary.totalGrossPay - salary.taxInfo.totalTax);

      // 정리
      await MonthlySalary.findByIdAndDelete(salary._id);
      await WorkSchedule.deleteMany({ userId: fourInsuranceEmployee._id });
      await User.findByIdAndDelete(fourInsuranceEmployee._id);
    });
  });
